...
public:
  ...
  static constexpr double L_perovskite = 950e-9; //300 m
  static constexpr double eps_rel = 24.1; // perovskite relative permittivity
  static constexpr double E_gap = 1.22; // perovskite band gap, eV
...
// MW
  ...
  static constexpr double G_max = 1.80e28; //original(1.65) maximum charge generation rate, m^{-3}s^{-1}
  static constexpr double k_b = 6e-17; // bimolecular recombination constant, m^3 s^{-1}

  // ETL (electron transporting layer) c60 TiO2
  static constexpr double L_etl = 20e-9; // m
  ...
  static constexpr double V_bi = 1.12; // 1.4 built-in voltage at short circuit, V
